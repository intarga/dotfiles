...
def create_left_prompt [] {
    ...
    let separator_color = (if (is-admin) { ansi light_red_bold } else { ansi red_bold })
    let path_segment = $"($path_color)($dir)"

    let path = $path_segment | str replace --all (char path_sep) $"($separator_color)(char path_sep)($path_color)"

    let branch_raw = git branch --show-current | complete

    let branch = if $branch_raw.exit_code == 0 { "\u{E0A0} " + $branch_raw.stdout } else { "\n" }

    $"($path) (ansi green)($branch)(ansi red)(whoami)(ansi blue)@(ansi red)(hostname)"
}
